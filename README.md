...
- **日曆**：依播出日期排列的月曆，每格可點進該集；日期讀自 `episode_dates.json` 或 `feed.xml`／RSS。
- **故事線**：四條敘事線（公司轉型／方法論／產品／外部生態）的垂直時間軸。
- **人物與組織**：炬識「三維立體組織」視覺化三層圖 —— HACOM（總部 ＋ 事業單位：資料／DBES／業務 Sales／顧問 Consulting）、Projecting（專案）、Communities（社群，附簡介）。每個單位顯示登場人數、上方有去重後的登場總人數。**點任一人物**會跳出卡片，內部成員顯示職涯演進（時間線）、隸屬單位與出現集數；外部來賓依「陣營」分組，顯示產業與「與 Anna／炬識的關係」。
- **詞彙**：依類別整理，★ 為跨集核心概念。
